...
def longest_ride(system):
    ...
    stops = set(boston.keys())
    return max([ride(a,b) for a in stops for b in stops], key=len)

    # candidate_stops = set()
    # for stops in boston.values():
    #     stopLst = stops.split()
    #     candidate_stops.add(stopLst[0])
    #     candidate_stops.add(stopLst[-1])
    # return max([ride(a,b) for a in candidate_stops for b in candidate_stops], key=len)
