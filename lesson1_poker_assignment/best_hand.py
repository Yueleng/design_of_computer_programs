...
def test_best_wild_hand():
    assert sorted(best_wild_hand_peter("6C 7C 8C 9C TC 5C ?B".split())) == ['7C', '8C', '9C', 'JC', 'TC']

    # Because this case returns random best hand we have to wrap it with hand_rank
    assert hand_rank(sorted(best_wild_hand_peter("TD TC 5H 7C 7D ?R ?B".split()))) == hand_rank(['7C', 'TC', 'TC', 'TD', 'TD'])
    
    assert sorted(best_wild_hand_peter("TD TC TH 7C 7D 7S 7H".split())) == ['7C', '7D', '7H', '7S', 'TD']
    return "test best wild hand passed"
