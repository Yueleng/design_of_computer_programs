...
def strategy_d(state):
    # based on the state, decide next move
    return hold_30_d2(state)

def hold_30_d2(state):
    """
    Hold at 20 pending. 
    Accept if me - you > -5
    Raise to double if me - you > 5
    """
    (p, me, you, pending, double) = state
    return ('accept' if double == 'double' 
                    and me - you >= -5 else 
            'decline' if double == 'double' and me - you < -5 else 
            'double' if double == 1 and me - you > 0 else
...
def clueless_d(state):
    "random choose any action based on state"
