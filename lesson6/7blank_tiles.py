        ...
        for L in hand:
            if L == '_': # allow blank
                for C in LETTERS_LOWER:
                    find_prefixes_cache(hand.replace(L, '', 1), pre+C, results)
            else:
...
def is_letter(sq):
    return isinstance(sq, str) and (sq in LETTERS or sq in LETTERS_LOWER)
            ...
            for L in hand:
                if L in LETTERS:
                    ...
                    for l in possibilities:
                        add_suffixes(hand.replace(L, '', 1), pre+l.lower(), start, row, results)
    return results
...
def calculate_score(board, pos, direction, hand, word):
    ...
    for (n, L) in enumerate(word):
        ...
        total += 0 if L in LETTERS_LOWER else POINTS[L] * letter_mult
